...
if(isset($_POST['register'])){ //blocks URL exploration
    ...
    if(empty($username) || empty($email) || empty($pswd) || empty($pswdConf)){
        header("Location: ../src/signup.php?error=emptyFields&username=".$username."&email=".$email."");
        exit();
    }
    elseif(!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $username)){ //probably only good for OG browsers
        header("Location: ../src/signup.php?error=invalidEmailUsername");
        exit();
    }
    elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)){ //probably only good for OG browsers
        header("Location: ../src/signup.php?error=invalidEmail&username=".$username."");
        exit();
    }
    elseif(!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
        header("Location: ../src/signup.php?error=invalidUsername&email=".$email."");
        exit();
    }
    elseif ($pswd !== $pswdConf){
        header("Location: ../src/signup.php?error=pswdNotMatching&username=".$username."&email=".$email."");
        exit();
    }
    else{
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){ //error handling
            ...
            if ($resultCheck > 0){
                header("Location: ../src/signup.php?error=userTaken&email=".$email);
                exit();
            }
            else{
                ...
                if(!mysqli_stmt_prepare($stmt, $sql)){ //error handling
                    ...
                    mysqli_stmt_execute($stmt);
                    header("Location: ../src/signup.php?success=signup");
                    exit();
                }
            }
        }
    ...
    header("Location: ../src/signup.php?error=urlExploration");
    exit();
}
?>